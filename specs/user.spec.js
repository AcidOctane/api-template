...
describe("Suite - users/ route", function()  {
  ...
  it.only("should get a user by id", async () => {
    const { statusCode, body } = await apiClient.user.getUserById(userInfo.body.user.id);
    expect(statusCode).to.be.eql(200);
    expect(body).to.be.a('object');
  ...
  it("shouldn't get unexisted user", async () => {
    const { statusCode } = await apiClient.user.getUserById(userInfo.body.user.id.slice(0, -3) + '000', false);
    expect(statusCode).to.be.eql(200);
  })
